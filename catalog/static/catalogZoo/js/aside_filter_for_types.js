...
document.addEventListener("DOMContentLoaded", function () {
    ...
    productListWrap.addEventListener("click", function (event) {
        const target = event.target;
        if (event.target.closest("#productsPrev")) {
            pass;
        }

        if (event.target.closest("#productsNext")) {
            pass;
        }

        if (event.target.matches("#pageNum")) pass;
    });
    ...
    function getLocalStorageData() {
        return {
            ...
            productCategoryId: document
                .getElementById("filter_item_active")
