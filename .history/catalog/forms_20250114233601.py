    ...
    def save(self, commit=True):
        ...
        for field in ['first_name', 'last_name', 'email']: 
            if self.cleaned_data.get(field): 
                setattr(user, field, self.cleaned_data[field]) password = self.cleaned_data.get('password1') if password: user.password = make_password(password) if commit: user.save() return user
